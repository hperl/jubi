...
module ProfileHelper
  ...
  def enum_radio_buttons(form, enum_field, options = {})
    if block_given?
      enums = ButRegistration.send(enum_field.to_s.pluralize).map {|e| [e.first, yield(e.first)]}
    else
      enums = ButRegistration.send(enum_field.to_s.pluralize).map {|e| [e.first, t(e.first)]}
    end
  ...
  def transaction_description(transaction)
    if transaction.class == Payment
      return "Einzahlung"
    elsif transaction.class == ButRegistration
